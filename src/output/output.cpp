...
namespace Mist{
  ...
  void Output::reinitPlaylist(uint64_t systemBoot){
    ...
    uint64_t curTime = M.packetTimeToUnixMs(currentStartTime, systemBoot);
  ...
  int Output::run(){
    ...
    reInitPlaylist = false; //< Reinit the playlist if we aren't appending to an existing one
    hasInitialSegment = false; //< Open the playlist once, after segmenting the first segment
      ...
      if (!currentPage.count(nxt.tid) || !curPage.count(nxt.tid) || !curPage[nxt.tid].mapped){
        ...
        if (!curPage[nxt.tid].mapped || curPage[nxt.tid].mapped[0] != 'D'){
          ...
          if (emptyCount % 100 == 0 && M.getLive() && Util::getStreamStatus(streamName) == STRMSTAT_OFF){
            Util::logExitReason(ER_SHM_LOST, "Stream source shut down");
            thisPacket.null();
            return true;
          }
          //every ~16 seconds, reconnect to metadata
          if (emptyCount % 1600 == 0){
            INFO_MSG("Reconnecting to input...");
            reconnect();
            if (!meta){
